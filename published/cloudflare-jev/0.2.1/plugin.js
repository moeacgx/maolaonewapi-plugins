...
function hasResponseFailure(body) {
  return (
    ...
    body.error != null ||
    (hasOwn(body, "errors") && (!Array.isArray(body.errors) || body.errors.length !== 0))
  );
}

function validateRequest(body) {
  if (!isObject(body)) throw new Error("请求必须是 JSON 对象");
  if (body.model !== MODEL) throw new Error("模型必须为 typesafe/jev");
  if (
    Object.keys(body).some(
      (key) => !["model", "state", "questions", "stream", "group"].includes(key),
    )
  )
  ...
  for (const id of Object.keys(body.questions)) {
    ...
    if (!isObject(question) || !["noul", "choice", "score"].includes(question.type))
    ...
    if (criteria != null && Object.values(criteria).some((value) => !isEntry(value)))
...
export function buildSubmitRequest(ctx) {
  ...
  if ((ctx.upstreamModel || ctx.model) !== MODEL) throw new Error("上游模型必须为 typesafe/jev");
  ...
  if (!match) throw new Error("Cloudflare 账户地址必须包含有效的 32 位 Account ID");
  if (typeof ctx.apiKey !== "string" || !ctx.apiKey || /\s/.test(ctx.apiKey))
    throw new Error("Cloudflare API 令牌无效");
  return {
    url: "https://api.cloudflare.com/client/v4/accounts/" + match[1] + "/ai/run",
    method: "POST",
    headers: {
    ...
    body: { model: MODEL, input: { state: request.state, questions: request.questions } },
  };
}
...
export function parseSubmitResponse(ctx, response) {
  ...
  if (hasResponseFailure(body)) throw new Error("Cloudflare 返回失败或无效结果");
  ...
  if (hasOwn(body, "state")) {
    if (body.state !== "Completed" || hasResponseFailure(body) || !isObject(body.result)) {
      throw new Error("Cloudflare 任务未完成或结果无效");
    }
    body = body.result;
    if (hasOwn(body, "state")) throw new Error("Cloudflare 任务未完成或结果无效");
  }
  if (hasResponseFailure(body)) throw new Error("Cloudflare 返回失败或无效结果");
  if (typeof body.model !== "string" || !body.model.trim()) throw new Error("响应缺少模型");
  ...
  for (const id of Object.keys(request.questions)) {
    ...
    if (answer.type === "choice") {
      if (typeof answer.choice !== "string" || !hasOwn(question.criteria, answer.choice))
        throw new Error("choice 结果不属于选项");
      answers.push([id, { type: "choice", choice: answer.choice, probabilities, confidence }]);
    } else {
      if (
      ...
      if (
        ...
        keys.some((key) => !hasOwn(answer.legend, key) || !isEntry(answer.legend[key]))
      ) {
  ...
  return {
    ...
    taskData: { model: body.model, answers: Object.fromEntries(answers), usage },
...
export function extractUsageOnComplete(_ctx, _result, body) {
  return { input_tokens: validateUsage(body).input_tokens };
}
...
export const native = {
  decodeSystemOne: function (ctx) {
    if (!ctx.body || ctx.body.kind !== "json") throw new Error("仅支持 JSON 请求");
    const requestBody = validateRequest(ctx.body.value);
    return { kind: "submit", model: requestBody.model, action: "systemone", requestBody };
  },
