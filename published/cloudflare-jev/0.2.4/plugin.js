...
// 仅按宿主提供的客户端路由身份过滤；合法入口的上游故障必须保持原统计。
export function shouldRecordPerformanceFailure(ctx, failure) {
  if (
    !isObject(ctx) ||
    !isObject(failure) ||
    ctx.pluginKey !== meta.key ||
    !MODELS.includes(ctx.model) ||
    !["transport", "http", "parse", "immediate"].includes(failure.stage) ||
    typeof ctx.method !== "string" ||
    !["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"].includes(
      ctx.method,
    ) ||
    typeof ctx.requestPath !== "string" ||
    !ctx.requestPath.startsWith("/") ||
    ctx.requestPath.includes("?") ||
    ctx.requestPath.includes("#")
  ) {
    return true;
  }
  if (
    ctx.method === "POST" &&
    ctx.requestPath === "/v1/task/plugins/" + meta.key
  ) {
    return true;
  }
  return meta.routes.some(
    (route) => route.method === ctx.method && route.path === ctx.requestPath,
  );
}
